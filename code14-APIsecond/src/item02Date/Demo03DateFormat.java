...
//        demo01();
    ...
    /*
        使用DateFormat类中的方法parse，把文本解析成日期，使用步骤12
        注意：
            public Date parse(String source) throws ParseException
            parse方法声明了一个异常叫ParseException
            如果字符串和构造方法的模式不一样，那么程序就会抛出异常
            调用一个异常的方法就必须处理这个异常，1、要么throws继续抛出这个异常，2、要么try catch自己处理
        下面使用第一种方法，简单：
                在报错的地方按 Alt+Enter 然后选择第一个 来声明
                再声明报错的demo02();
    */
    private static void demo02() throws ParseException {
        //1、创建SimpleDateFormat对象，并向构造方法中传递指定的模式
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy年MM月dd日 HH时mm分ss秒");
        //2、调用SimpleDateFormat对象中的parse方法，把符合构造方法中模式的字符串，解析成Date日期
        Date date = sdf.parse("2019年05月26日 20时11分57秒");
